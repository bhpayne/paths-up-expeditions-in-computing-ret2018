...
import serial # sudo -H pip3 install pyserial
import time
import array
...
def setup_csv(csvStr=None):
    ...
    headers = str(u'pulseWaveform'+','+u'pulseRate'+','+u'time')
...
def save_to_csv(csvFileName, cmsData):
    with io.open(csvFileName + '.csv', 'a', newline='') as f:
        row = str(str(cmsData['pulseWaveform'])+','+str(cmsData['pulseRate'])+','+str(cmsData['time']))
        f.write(row)
        f.write(u'\n')
